...
- `.once`: fires once; after completion it has no next-due. **It is treated as COMPLETED, not Paused (feedback pass 5, item 3)** — a completed once (and a done `.unscheduled` reminder) leaves the active med/task/reminder lists, reads with the green "done" treatment, and appears in the patient's "Completed today" section (and the Log). Distinguished from a once **paused before completing** (which keeps a non-nil `nextDueAt` and stays active-Paused) via the pure Core predicate `SchedulingEngine.isCompletedTerminal(schedule:nextDueAt:lastCompletedAt:)` — derived from existing state, no new persisted field. (The engine still exposes `shouldAutoPauseAfterCompletion` as a fact, but the app no longer auto-pauses on completion.)
...
**Reduction surfacing is non-blocking (design pass, feedback item 2).** Reminder reduction is informational, not an error, so it no longer occupies the top-of-screen banner (which obstructed controls). Instead it drives: (a) a **one-time-per-change**, dismissible alert on app open and whenever the reduction condition first becomes true or its details change; and (b) a small **persistent, tappable nav-bar indicator** on the Board while reduction is active, which re-shows the details on demand. Every task remains on the board. **Persistence-error banners are unaffected and keep their existing priority.** **Specific messaging (feedback pass 5, item 4):** the indicator surfaces only for reductions a nurse cares about — a **dropped early reminder** or **grouped alerts** — not for tail-only repeat-ping trimming (pre/due/floor stay intact). When early reminders are dropped it says so explicitly: *"Some early reminders were trimmed to stay within iOS notification limits. Every task is still on your board and its due-time alert still fires — only the early heads-up was dropped, furthest-out doses first."* A nurse relying on a 30-min ping can thus discover it was dropped.

### 4.4 Interruption level
`.timeSensitive`. No Critical Alerts in v1 (v2 candidate). **Deployment note (feedback pass 5, item 4):** the `com.apple.developer.usernotifications.time-sensitive` **entitlement** is removed from the App and Watch entitlements so a **personal/free Apple team** can sign and install (it was the "app cannot be installed at this time" blocker). The code still requests `.timeSensitive`, which degrades to normal delivery without the entitlement; break-through of Focus/DND requires re-adding the capability under a **paid** team.
...
- **Add/Edit Task form:**
  ...
  - **Near-due creation note (design pass, feedback pass 5 item 3):** when the effective first due minus the lead is already in the past, the schedule preview says so — "Due 5:00 PM · too soon for a 30-min early reminder — first alert at due time" — so a skipped pre-alert is **visible intent, never silent** (the item-1b cause). Suppressed when the task's notifications are muted.
...
- **Unscheduled reminders (design pass, feedback pass 5 item 1).** A new `ScheduleType.unscheduled` (additive, decode-safe — corrupt/unknown payloads still quarantine to `.needsRepair`, never silently `.unscheduled`) lets a `.reminder` have **no schedule**: no due time, no reminders ever planned, excluded from Schedule projections and the Board's due-ordering. The Add/Edit picker offers **"No schedule"** for reminders only (and defaults reminders to it); meds/care stay schedule-required. Rendered as a plain checklist item — status `.unscheduled` ("No time"), and the task sheet shows **Done / Edit / Delete** only (no Snooze/Skip/Pause). In patient detail, unscheduled reminders sit under a **"Reminders · no time"** section; **[user decision]** they also appear on the **main Board** under the patient card (the original spec grouped them in patient detail only — surfacing them on the Board was the user's call).
