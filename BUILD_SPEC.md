...
`.timeSensitive`. No Critical Alerts in v1 (v2 candidate). The scheduler sets `content.interruptionLevel = .timeSensitive` and the `com.apple.developer.usernotifications.time-sensitive` **entitlement is present** in the App and Watch entitlements (restored under the paid Apple Developer team — feedback pass 6), so reminders break through Focus/DND with user permission. (It was temporarily removed during free-tier signing; that workaround is reverted.)
